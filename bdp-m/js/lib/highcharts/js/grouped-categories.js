...
// Builds reverse category tree
    function buildTree(cats, out, options, parent, depth) {
        ...
        while (len--) {
            ...
            if (parent&&Object.prototype.toString.call(cat) === '[object Object]')
                cat.parent = parent;
    ...
    axisProto.render = function () {
        ...
        while (i <= depth) {
            ...
            part = horiz ?
                ...
                [offset, top+1, offset, bottom];

            // first tick line horizion
            addGridPart(d, part);
            i++;
        }
...
// Override methods prototypes
    tickProto.addLabel = function () {
        ...
        // set label text
        if (category.name)
            this.label.attr('text', category.name.replace(/^\s+/, '').replace(/\s+$/, ''));
...
// set labels position & render categories grid
    tickProto.render = function (index, old, opacity) {
        ...
        // group label vertical tick line
        addGridPart(grid, gridAttrs);
        ...
        while (group = group.parent) {
            ...
            if (grid) {
                ...
                // first level vertical tick line
                addGridPart(grid, gridAttrs);
            }
